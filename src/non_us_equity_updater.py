...
def main():
	...
	logger = logging.getLogger( 'non_us_equity_updater' )
	...
	for exchange_code in exchange_codes:
		df = pd.read_csv('/data/non_us_equities_' + exchange_code + '_' + run_date + '.csv')
		dfs.append( df )
  
	non_us_eq = pd.concat( dfs, ignore_index=True )

	fin_types = secdb.get_fin_types()
	trading_places = secdb.get_trading_places()

	symbol_adds = non_us_eq[non_us_eq['figi'].isin(instruments['figi']) == False].copy()
	logger.info( f'{len(symbol_adds)} new symbols' )

	symbol_dels = instruments[instruments['figi'].isin(non_us_eq['figi']) == False].copy()
	logger.info( f'{len(symbol_dels)} symbol deletes' )

	compare = pd.merge( instruments[['figi','ticker','name']], non_us_eq[['figi','symbol','name']], left_on='figi', right_on='figi', how='inner' )
